...
export function AnimeNavBar({ items, className }: NavBarProps) {
  ...
  useEffect(() => {
    const getActiveTab = () => {
      ...
      if (hash) {
        const correspondingItem = items.find(i => i.url.endsWith(hash));
        if (correspondingItem) return correspondingItem.name;
      }
  ...
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, url: string, name: string) => {
    if (url.startsWith('/#')) {
      ...
      if (element) {
        ...
        window.history.pushState(null, '', url.substring(1));
        setActiveTab(name);
      }
    } else {
       setActiveTab(name);
  ...
  const allNavItems = [{name: 'Home', url: '/', icon: Logo as any}, ...items];
  ...
  return (
    <div className="fixed top-5 left-0 right-0 z-[9999]">
      <div className="flex justify-center pt-6">
        ...
        >
          <div onMouseEnter={() => setHoveredTab('Home')} onMouseLeave={() => setHoveredTab(null)} onClick={() => setActiveTab('Home')}>
            <Logo />
          </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive && !item.highlighted && "text-white",
                  item.highlighted && "bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground flex items-center gap-2"
                )}
              >
                ...
                <AnimatePresence>
                  {(isHovered || (activeTab === 'Home' && item.name === 'Home')) && !isActive && !item.highlighted && (
                    <motion.div
                ...
                {(isActive || (activeTab === 'Home' && item.name === 'Home' && !isMobile)) && !item.highlighted && (
                  <motion.div
